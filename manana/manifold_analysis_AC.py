...
def manifold_analysis(XtotT, kappa, n_t, t_vecs=None, n_reps=10):
    '''
    Carry out the analysis on multiple manifolds, but not considering the correlations across manifolds.
    ...
    for i in range(num_manifolds):
        ...
        Xr0_norm = np.linalg.norm(Xr0)
        X_norms.append(Xr0_norm)
        # Center normalize the data
        Xrr = (Xr - Xr0.reshape(-1, 1))/Xr0_norm
        XtotInput.append(Xrr)
